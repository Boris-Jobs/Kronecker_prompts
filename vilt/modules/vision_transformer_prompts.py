    ...
    def forward(self, x, mask=None, prompts=None, learnt_p=False, prompt_type="input"):
        ...
        if prompts is None or prompt_type == "input" or prompt_type == "kronecker":
            ...
            q, k, v = (
                ...
                torch.cat([prompts_k, qkv[:, :, 1, :]], dim=1)
                .reshape(B, N + P, 12, C // 12)
                .permute(0, 2, 1, 3),
                torch.cat([prompts_v, qkv[:, :, 2, :]], dim=1)
                .reshape(B, N + P, 12, C // 12)
                .permute(0, 2, 1, 3),
            )
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, prompts=None, learnt_p=False, prompt_type="input"):
        if prompts is not None and (
            prompt_type == "input" or prompt_type == "kronecker"
        ):
...
def _create_vision_transformer(variant, pretrained=False, distilled=False, **kwargs):
    default_cfg = default_cfgs[variant]  # 从比如"vit_base_patch32_384"加载默认配置
