...
LOG_FILE="/scratch/project_2007023/boris/missing_aware_prompts/missing_aware_prompts/result/imdb_train_noprompt_${TIMESTAMP}.txt"

# srun -N 1 -n 1 -t 00:15:00 --mem-per-cpu=32G --gres=gpu:a100:2 \
#      -p gputest --account=project_2002243 python3 checkpoint_check.py \
#      | tee "$LOG_FILE"

srun -N 1 -n 1 -c 3 -t 20:00:00 --mem-per-cpu=32G --gres=gpu:a100:2 \
