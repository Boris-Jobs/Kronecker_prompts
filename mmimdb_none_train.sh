...
LOG_FILE="/scratch/project_2007023/boris/missing_aware_prompts/missing_aware_prompts/result/${TIMESTAMP}_mmimdb_none_train.txt"
...
srun -N 1 -n 1 -c 3 -t 33:33:33 --mem-per-cpu=32G --gres=gpu:a100:2 \
     ...
     exp_name=${TIMESTAMP}_mmimdb_none_train \
     | tee -a "$LOG_FILE"
...
if [ $? -eq 0 ]; then
    ...
    srun -N 1 -n 1 -c 3 -t 01:00:00 --mem-per-cpu=32G --gres=gpu:a100:2 \
         ...
         none_prompts \
         step50k \
         load_path=/scratch/project_2007023/boris/missing_aware_prompts/missing_aware_prompts/result/${TIMESTAMP}_mmimdb_none_train_seed0_from_vilt_200k_mlm_itm/version_0/checkpoints/last.ckpt \
         with_delta_infer=True \
         exp_name=${TIMESTAMP}_mmimdb_none_test_with_delta \
         test_only=True \
         | tee -a "$LOG_FILE"
